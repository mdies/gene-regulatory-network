...
/*									   */
/*    From https://www.uni-oldenburg.de/fileadmin/user_upload/physik/ag/\  */
/*               compphys/download/Alexander/dpg_school/r1279.c 	   */
/*    And modified by Marta Dies (martadies@gmail.com)			   */
/***************************************************************************/
...
{
/* ioffset is  increased by (1 mod 2048) for
	each random number which is called.
*/
// (Marta Dies) - I need to eliminate possible zeros, otherwise they would mess with Anderson's
// algorithm stochastic simulation code 
 do { //First, "do" is executed
    ioffset = (ioffset + 1) &  2047;
    irand[ioffset] = irand[index1[ioffset]] * irand[index2[ioffset]];
 } while ( irand[ioffset] >> 1 == 0); //The condition is evaluated in the end => whatever code inside the "do"
				      //will be executed at least ONCE, with independence on the initial value
				      //of "irand[ioffset] >> 1"
 return (irand[ioffset] >> 1) * INV_MAXINT;// Multiplying by the Maximum Integer inverse in 
 					   // order to normalize (r \in [0,1])
}
